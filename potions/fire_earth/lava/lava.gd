...
func _on_area_shape_entered(area_rid, area, area_shape_index, local_shape_index):
	if area and weakref(area).get_ref() and ('Breakable' in area.get_parent().name or 'Durable' in area.get_parent().name or 'Boundaries' in area.get_parent().name or 'NoLava' in area.get_parent().name):
		queue_free()
